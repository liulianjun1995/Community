...
@section('content')
    ...
    <fieldset id="articleList" class="layui-elem-field layui-field-title sys-list-field" style="display:block;text-align: center">
        <legend id="articleBoxTitle">禁言列表</legend>
        <div class="layui-field-box">
            <div id="articleContent" class="">
                <table style="width: auto;margin: 0 auto" class="layui-table" lay-even="">
                    ...
                    <tbody>
                    @foreach($gagUsers as $gagUser)
                        <tr>
                            <td>{{$gagUser->user->id}}</td>
                            <td>{{$gagUser->user->name}}</td>
                            <td>{{$gagUser->created_at}}</td>
                            <td>
                                <button class="layui-btn layui-btn-small layui-btn-normal" onclick="delGag(this,{{ $gagUser->user->id }})">
                                    删除
                                </button>
                            </td>
                        </tr>
